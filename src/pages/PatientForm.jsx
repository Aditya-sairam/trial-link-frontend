...
export default function PatientForm() {
  ...
  const { id } = useParams();               // present only on /patients/:id/edit
  ...
  // ── In edit mode: fetch existing patient and pre-fill the form ──
  useEffect(() => {
    if (!isEditMode) return;
    getPatient(id)
      .then((data) => {
        // Convert date objects to ISO strings for HTML date inputs
        const normalized = JSON.parse(JSON.stringify(data));
        reset(normalized); // pre-fills all form fields at once
      })
  ...
  const handleNext = async () => {
    ...
    console.log("Triggering step:", currentStep, "fields:", fieldsToValidate);
    const valid = await trigger(fieldsToValidate);
    console.log("Valid:", valid);
    console.log("Errors:", JSON.stringify(methods.formState.errors, null, 2));
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex justify-between mt-6">
            ...
            {isLast ? (
              ...
              <button type="button" onClick={(e) => { e.preventDefault(); handleNext(); }} className="btn-primary">
                Next →
              </button>
            )}
          </div>
